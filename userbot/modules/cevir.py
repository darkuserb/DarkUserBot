...
async def cevir(event):
    ...
    if islem == "foto" or islem == "photo":
        ...
        try:
            await event.client.send_file(event.chat_id, "out.gif",reply_to=rep_msg, caption=LANG['WITH_BOSS_GIF'])
        except:
        ...
        try:
            ...
            os.remove(video)
            return await event.edit('`Səsə çevirə bilmədim!`')

        await event.delete()
        os.remove("out.mp3")
        os.remove(video)
    elif islem == "voice":  #credit:miri
        caption = "@BossUserBot ilə səsli mesaja çevirildi."
        rep_msg = await event.get_reply_message()
        sarki = rep_msg.text
        if rep_msg.audio:
            await event.edit(f"__Səs Endirilir...__")
            indir = await rep_msg.download_media()
            await event.edit(f"__Səsi Endirdim, səsli mesaj olaraq göndərirəm...__")
            ...
            if path.isfile("unvoice.ogg"):
                ...
                await event.edit("`Səs, səsli nota çevirilə bilmədi!`")
            remove(indir)
            return
    else:
        await event.edit(LANG['INVALID_COMMAND'])
...
CmdHelp('cevir').add_command(
    'çevir photo', '<yanıt>', 'Stickeri şəkilə çevirir.'
).add_command(
    'çevir gif', '<yanıt>', 'Videoyu gifə çevirir.'
).add_command(
    'çevir audio', '<uşaq/robot/earrape/sürətli/parazit/yankı>', 'Səsə effekt edər.'
).add_command(
    'çevir mp3', '<yanıt>', 'yanıtladığınız videoyu mp3\'ə çevirir.'
).add_command(
    'çevir voice', '<yanıt>', 'yanıtladığınız mp3\'ü səs atmış kimi göstərir.'
).add()
